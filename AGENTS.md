...
5. **下面 12 个文件是「闸」，动它们之前必须先请示维护者。** 它们是判定对错的东西 ——
   ...
   **一条豁免**：升版脚本 `bump_version.py` 改写的 profile 头注与文档活指向属机械动作，不算越界
   ...
   `all.sh` 每次跑完会在末尾回一句实出的陈述：无放行在场时是「本批待落地改动未触碰闸门文件」，
   有放行在场时是「本批待落地改动未触碰其他闸门文件」——两种句式逐字照代码出声（查：未提交与未推送的提交两面，
   该仓没有上游时退一步只看「最近一次提交（该仓无上游）」；只报不判负，无需翻 diff）。
   **唯一的放行例外**是 `all.sh` 里的 `once_released()`：它只认 `skill/tests/surge/check_links.py` 这一件，
   判据改为**工作区该件带着标记、且该标记尚未进入 HEAD 时对这一件放行**；标记被删、或授权那一批一落地（标记进 HEAD），豁免立刻失效；看不见 HEAD 一律不放行（豁免生效期间，
   放行时另出一行 ℹ️ 点名该件）。**后来者不许照学这个模式给任何文件开后门**：加名单项、放宽判据、
   给自己放行，一律按本条先请示维护者。
