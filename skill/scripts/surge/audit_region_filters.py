...
审什么
──────
`surge/profiles/routing_v3.2.conf` 里有 7 个地区组，它们的关键词是**两份拷贝**：
...
用法：
    python skill/scripts/surge/audit_region_filters.py surge/profiles/routing_v3.2.conf
    python skill/scripts/surge/audit_region_filters.py surge/profiles/routing_v3.2.conf -v
"""
...
# 7 个地区组的名字 —— 与 routing_v3.2.conf 里的组名逐字对应。
# ⚠️ 若你在 routing_v3.2.conf 里改了组名，这里也要改（脚本会报缺失，不会静默通过）。
