...
│   ├── scripts/{surge,egern}/   # 审计脚本（Surge 4 + 1 共享模块 / Egern 9 + 1 共享模块）
...
3. python skill/scripts/surge/check_surge_dns.py  surge/profiles/routing_v3.conf  → exit 0
4. （改了地区关键词时）python skill/scripts/surge/audit_region_filters.py surge/profiles/routing_v3.conf  → 9 passed
