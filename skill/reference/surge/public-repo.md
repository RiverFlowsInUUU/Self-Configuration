...
├── CHANGELOG.md                 # 合并成仓的装配史（内容迭代史在各内核 CHANGELOG）
├── LICENSE                      # MIT
├── .gitignore
...
└── surge/                       ── 本内核的全部产品物 ──
    ...
    ├── CHANGELOG.md              # 本内核的迭代史（Keep a Changelog，时间倒序）
    ...
    ├── docs/                     # 01–08 编号系列 + 11-分流版设计
    └── DetailsReadme/
        └── DetailsReadme.md      # 完整技术文档（18 节）
```

> ⚠️ **合并带来的三处职责变化**（改动前先看这里，别按旧仓的习惯放文件）：
...
| `surge/CHANGELOG.md` | 面向用户的变更 | **工作过程、内部重构** |
