...
```
Self-Configuration/
├── README.md                    # 门面：内核选择 / 四份订阅地址 / 五类泄露面 / 组结构 / 目录
...
└── surge/                       ── 本内核的全部产品物 ──
    ...
    │   ├── routing_v3.1.conf     # 分流版（带注释）—— 改这份
    │   └── routing_v3.1.min.conf # 分流版（纯配置）—— 导入用
    ├── docs/                     # 01–08 编号系列 + 11-分流版设计（07 末节 = 本内核合并前的迭代史）
    └── DetailsReadme/
        └── DetailsReadme.md      # 完整技术文档（18 节）
```

> ⚠️ **合并带来的四处职责变化**（改动前先看这里，别按旧仓的习惯放文件）：
...
**对使用者有影响吗？** 内部重构、仓库运维、行尾规范化 —— 用户无感，不进 README 与 `docs/`。
它们进日志的 **`共享层` 小块**（日志同时记装配与检查层，见下面的体例）。

### 日志体例（2026-09-24 定）

写在根 `CHANGELOG.md`，三条：

1. **一天一段**：当天后续改动往那一段里增补，**不开第二个同名日期段**。
2. **段内分块**：`### Surge` · `### Egern` · `### 共享层`，当天没动的内核不写。
3. **一条一句**：只答「改了什么 · 哪里没动 · 验收」。推理过程、请示与授权、当时哪里判错了、
...
当真链接解析，相对 `skill/reference/surge/` 必然失效。）
...
#### 2.4.3 首页不列规则集 —— 读者只关心「能实现怎样的分流」

规则集属**实现侧**：用了哪些 `.list`、从哪个仓库拉、顺序怎么排。使用者真正关心的是
**分流结果** —— 有哪些组、按地区还是按应用分、哪类流量去哪。

| | 首页（`README.md`） | 组件页（`docs/`） |
|:--|:--|:--|
| 视角 | 产品：能实现怎样的分流 | 实现：用了什么组件 |
| 顺序表 | 「匹配什么 → 去向」（白名单域名 / 广告域名 / 按应用 / 国内…） | 逐条列出规则集名与参数 |
| 规则集清单 | ❌ 一个都不出现 | ✅ 文件名 · 去向 · 来源 URL · 刷新方式 |
| 来源仓库 | ❌ | ✅ 含图标、数据库、许可 |

**判据**：首页上这句话，对读者**用**这份配置有没有帮助？
「`OpenAI.list` → `ChatGPT`」没有意义（使用者不能按文件名分流）；
「AI 应用走 `AI` 组，面板上可改道」才有意义。

实测（2026-09-22）：三仓首页各挂着「📚 规则来源」段 + 「分流版的应用规则」表 + 「排序约束」，
...
本仓库有**两份配置**：`lazy.conf`（懒人版）与 `routing_v3.1.conf`（分流版）。
...
3. python skill/scripts/surge/check_surge_dns.py  surge/profiles/routing_v3.1.conf  → exit 0
4. （改了地区关键词时）python skill/scripts/surge/audit_region_filters.py surge/profiles/routing_v3.1.conf  → 9 passed
