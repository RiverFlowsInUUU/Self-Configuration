...
末尾放「可先看两个姐妹仓」。用户同判 —— 「这句话对读这个项目的人有必要吗？」
...
首页只在「文件结构 / 更多文档」里留一个链接即可（2026-09-25 下半两节合一后，该去处现为唯一的「📖 按需查阅」表）。
...
⚠️ 从前这行要**手工补回**（生成器剥注释）；2026-09-24 起由 `skill/tests/make_min.py` 生成，
它按锚点把这类有语义的注释继承过去，跑 `--apply` 就行。
