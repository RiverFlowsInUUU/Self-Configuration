...
排除口径（默认）：
    ...
    沿革                    "哪一版改了什么"的记述。两份内核 CHANGELOG 的正文已并入
                            {surge,egern}/docs/07-文件版本沿革.md 末节，那里的历史条目靠这一条排除
    docs/体检报告.md        审计读数快照：每节写的都是**那一轮**的观测值，不是活指向
    docs/日志旧版原文.md    合并前日志的只读存档，整篇是历史原文
...
def main():
    ...
    EXC = [re.compile(r"(^|/)CHANGELOG\.md$"), re.compile("沿革"),
           # 读数快照类文档：里面「v3.1 那轮的读数是 0 high / 2 low」这类句子是**当时的观测**，
           # 跟着升版改就等于把证据改成现在。它们会经 `skipped` 列进第 5 段的人工清单，不静默。
           # （2026-09-24 v3.1→v3.2 那轮就是这两份被机器改写完再手工 `git checkout --` 回滚的。）
    ...
    print("   · CHANGELOG.md（根那一份，2026-09-24 起是唯一的一份）新章节 —— 升版说明是人写的文案")
    print("   · docs 里的「版本沿革」类记述 —— 历史口径，改了就是篡改")
    print("     （`docs/体检报告.md` 与 `docs/日志旧版原文.md` 同属此列：里面出现的旧版本号是当时的读数，"
          "已随排除一并跳过；若确实有活指向混在中间，按下面「排除路径里仍有旧名」逐条人工确认）")
