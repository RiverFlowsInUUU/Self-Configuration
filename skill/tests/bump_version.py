...
这里把那部分机械劳动收进脚本，**人的判断留在脚本之外**：
    ...
    · 它也**不**碰与两个前身仓的逐字节对账 —— 那需要源仓的克隆在旁边，属维护者本地的合并
            验收（边界同 `all.sh` 头部所述），升版后新增的四个文件名要在那边人工确认。
...
⚠️ 关于源码里的换行：本文件刻意用 NL / CRLF 两个常量表示换行，不写字面转义 —— 一旦有人用
   ...
   另：所有改写都按原文件的换行写回（`core.autocrlf=true` 下工作树与提交内 blob 换行不同）。
"""
import argparse
import io
import os
...
def main():
    ...
    print("   · 与源仓的逐字节差异基线：会多出四个新文件名，需维护者在本地确认后更新（不在本仓自检里）")
