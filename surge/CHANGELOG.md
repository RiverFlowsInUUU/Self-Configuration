...
- 📚 **首页下沉规则集信息，新开专题接住** —— 首页原有三处规则集层面的内容：「📋 规则顺序」表（`surge-white-guard.list` / `AI.list` / `direct.txt` / `GEOIP,CN` 等）、「分流版的应用规则」表（13 行「规则集 → 去向」）与「📚 规则来源」整节。规则集属**实现侧**，读者只关心「能实现怎样的分流」，于是：
  - 📋 「规则顺序」改为 **「分流顺序」** —— 列「匹配什么 → 去向」（白名单域名 / 广告域名 / 按应用 / 游戏机主机名 / Apple / 内网 / 国内 / 其余全部），首页不再出现规则集文件名与 `GEOIP` 这类规则类型；
  - 📚 「分流版的应用规则」表与「三条排序约束」撤下首页（**应用组的默认出口表留下** —— 那是组级信息，使用者要按组改道）；
  - 📚 「规则来源」整节撤下首页；
  - 🆕 新开 [`docs/12-规则集与来源.md`](../docs/规则集与来源.md) —— 10 份共用规则集 + 13 条分流版专有规则集的「作用 / 去向 / 来源」、两版逐条匹配顺序（含参数）、四条排序约束、素材与许可；
  - 🔗 首页只在「文件结构」与「更多文档」里各留一个入口（篇数 11 → **12**）。
- 📘 **公开仓规范补 §2.4.3「首页不列规则集」** —— §2.4.1 管「详细内容下沉」、§2.4.2 管「标题不出现『原理』」，本次再补一条：**连「有哪些规则集」都不放首页**。判据是「首页上这句话对读者**用**这份配置有没有帮助」——「`OpenAI.list` → `ChatGPT`」没有意义（使用者不能按文件名分流），「AI 应用走 `AI` 组、面板上可改道」才有意义。规则集文件名 / 来源仓库统一进 `docs/` 组件页。起因：用户先要求来源段下沉，随即补充「不仅是规则集的来源，而且是有哪些规则集……我认为都没必要放在首页的 README 里面」。

- 🧹 **README「分流」这块压成一张表** —— 原先散着 4 张表（懒人版组表 · 分流版「层 / 组 / 选路」表 · 应用组默认出口表 · 分流顺序表），「出口」「选路」「匹配什么」混着讲，读者得自己拼。改为**一节一表**：`## 🧭 分流组`，左列懒人版、右列分流版，**按配置自上而下**逐行列出每个策略组，懒人版没有的记 `-`。出口与选路一律不写。
  - 📊 组顺序由 `profiles/*.conf` 的 `[Proxy Group]` 段**实时提取**（懒人版 3 组 / 分流版 26 组），不是手抄；脚本带断言「懒人版相对顺序在合表后不被破坏」。
  - 🗑️ 撤下：「应用组的默认出口」表、分流版「层 / 组 / 选路」表、「📋 分流顺序」表；原「两份配置」节补回规模口径（3 组 / 14 条 · 26 组 / 27 条），免得数字随表一起丢。
  - 🔗 同步修 `docs/04` §5 的失效指路（`../README.md#-分流顺序` → `docs/11` §5 + `docs/12`）。
  - 📌 起因：用户「readme 分流这块乱的让我恶心……你就列一个表格，按照配置把从上到下的分流组分别是啥列出来，一列是懒人版，没有的就写个 `-`，一列是分流版……什么出口、选路什么的不重要不用写」。
  - 🔢 顺手修一处过期读数：badge `Rules` `13 | 26` → **`14 | 27`**（上一轮广告清单补第二条时规则数已增至 14 / 27，badge 漏改）。

### 修复

- 🐛 **`check_links.py` 锚点算法三处修正 —— 此前既误报又漏报** —— 本脚本是仓内链接质量的门禁，它自己错了比链接错了更危险。用 GitHub 真正使用的 `github-slugger` 逐条对拍（三仓 638 个标题）后修正：
  - `_` 下划线**应保留**（不在 github-slugger 的删除字符类里）。原实现按白名单丢弃，于是 `docs/04-模板逐段讲解.md#4-policy_groups-段` 这类**写对了的**锚点被判「不存在」（假阳性）。
  - FE0F 变体选择符**应保留**（属 Mn 组合符，不是符号）。原实现丢掉它，而 GitHub 上 `## 🛡️ Surge 配置模板` 的真实锚点是 `#️-surge-配置模板`（`#` 后有一个**看不见**的字符）—— 判据与事实相反。
  - 同名标题应加 `-1` / `-2` 后缀。原实现不计数，指向第二个 `## 新增` 的 `#新增-1` 会被漏判。
  - 另修一处文件与注释不符：注释声明「跳过 fenced code block」，代码里从未实现 —— 现已落地，代码块里的示例链接不再进统计。
  - ✅ 修完对拍 **clash 27 / surge 402 / egern 209 个标题，与 `github-slugger` 输出 100% 一致**；新增 `--selftest` 把上述易错点固化成用例，防止回退。
- 🔗 **修两处因首页下沉而失效的仓内指路** —— README 章节改名与撤下会让引用它的链接**静默失效**（GitHub 不报错，读者点 404），本轮由 `check_links.py` 抓出：`docs/04` §5 的 `../README.md#-规则顺序` → `#-分流顺序`；`docs/10` 的「规则集来源清单见 README 的 📚 规则来源」→ 改指 [`docs/12`](../docs/规则集与来源.md)。
