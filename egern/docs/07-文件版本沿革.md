...
> | 🧭 **分流版** | `routing`（当前版）· 存档 `routing_v1` … `routing_v3.2` | 按应用 + 按地区分流。**顶层 `routing.yaml`（首行 `#! version=routing_v3.3`）为当前推荐** |
>
...
|:----:|:------:|:----:|:--------:|:--------------:|:-----|
| `lazy` 🪶（`lazy_v1.2`） | 4 | 10 | 1 | 2 | 懒人版 —— 一个出口，只做防泄露 |
| **`routing`** 🧭（`routing_v3.3`） | 26 | 24 | 1 | 2 | **推荐** —— 分流版，按应用 + 按地区 |

- 🪶 **`lazy`（懒人版）** —— 只要防泄露、不要分流就选它：`Proxy` / `AI` / `AD` 三个分流组
  + 一个隐藏的订阅槽位 `Airport`，共 4 组、10 条规则，另带 2 条占位节点。
  ⚠️ **订阅槽位与占位节点至少填一项**，否则所有走代理的流量都不通。
- 🧭 **`routing`（分流版 · 推荐，首行 `#! version=routing_v3.3`）** —— AI / 流媒体 / 地区组齐全，26 组 / 24 条规则；
...
能力已被当前版 `routing`（`routing_v3.3`）覆盖，**新用户不用看**：

`routing_v1` → `routing_v2` → `routing_v2.1` → `routing_v2.2` → `routing_v2.3` → `routing_v2.4` → `routing_v3` → `routing_v3.1` → `routing_v3.2` → **当前版 `routing`（`routing_v3.3`）**
...
| **当前版 `routing`**（`routing_v3.3`） | 26 / 24 | ① `proxies` 由空列表变成 **2 条占位节点**（`Node-A` → `Proxy` **末位**、`Node-B` → `AI` **首项**）；② `Spotify` / `YouTubeMusic` 首项 `Proxy` → **`USA`**；③ `dns` 段、其余分组与 24 条规则**逐字未动**；④ `routing_v3.2` 进 `config_old/`，**顶层文件名与订阅地址不变** |
...
### 当前版 `routing` ← `routing_v3.2`（2026-09-26）
...
| `routing_v3` ~ 当前版 `routing_v3.3` | 0 high / 2 low / 24 ok（四版静态读数相同） |
