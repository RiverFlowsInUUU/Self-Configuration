...
> `profiles/` 里一共 18 份配置（9 个版本 × 带注释 / 纯配置两份），分成**两条线**：
>
> | 线 | 文件 | 定位 |
> |:--|:-----|:-----|
> | 🪶 **懒人版** | `lazy` | 一个出口，只做防泄露。**不挂版本号**，长期沿用 |
> | 🧭 **分流版** | `routing_v1` … `routing_v3.1` | 按应用 + 按地区分流。**`routing_v3.1` 为当前推荐** |
>
...
> **文件版本**用 `v`（文件名 `routing_v1`…`routing_v3.1`）；
...
| **`routing_v3.1`** 🧭 | 26 | 24 | 1 | **推荐** —— 分流版，按应用 + 按地区 |

- 🪶 **`lazy`（懒人版）** —— 只要防泄露、不要分流就选它：只留 `Proxy` / `AI` / `AD`
  3 个组、10 条规则。⚠️ 它没有 `Airport-*` 订阅槽位，**`Proxy` 必须自己填节点**，否则所有走代理的流量都不通。
- 🧭 **`routing_v3.1`（分流版 · 推荐）** —— AI / 流媒体 / 地区组齐全，26 组 / 24 条规则；22 条 `rule_set` 都带
...
能力已被 `routing_v3.1` 完全覆盖，**新用户不用看**：

`routing_v1` → `routing_v2` → `routing_v2.1` → `routing_v2.2` → `routing_v2.3` → `routing_v2.4` → `routing_v3` → **`routing_v3.1`**
...
- **防泄露结构与分流版同构**：`dns` 段去掉行尾注释后与 `routing_v3.1` 逐字相同
