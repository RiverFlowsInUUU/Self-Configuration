...
| 加密 DNS | `encrypted-dns-server = https://…` | `dns.upstreams.<组名>` | 两侧都必须写 **IP 字面量** |
...
2. **源仓文档中的四处过时内容**（合并版已修，源仓未改动）：
   - Surge `docs/12` 的懒人版顺序表仍列着已删除的 3 条游戏机主机名；
   - Egern `docs/11` 第 84–101 行是一段编辑残留的孤立表格碎片；
   - Egern `CHANGELOG` 头部与 `DetailsReadme` 各有 1 条指向**已撤下的 README 章节**的锚点
     （`#-两份配置` / `#-分流版`）—— GitHub 对失效锚点**不报错**，点了就是页面顶部，
     合并版分别改指 [`egern/docs/07-文件版本沿革.md`](../egern/docs/07-文件版本沿革.md)
     与 [`docs/规则集与来源.md`](规则集与来源.md)。
   前两处是在**重建**那两篇共享文档时发现并直接修掉的；后两处由链接 / 锚点检查器抓出。
   （Surge 源仓的锚点全部有效；Egern 源仓仅此 3 条失效，已逐一核对。）
