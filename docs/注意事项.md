...
| 📜 **历代版本进 `config_old/`** | 订阅地址用固定名，被替代的按版本号进 `profiles/config_old/`，逐版头部写明「为什么改」：Egern 侧 `routing_v1` → `v3.1` 共 8 版 ×2、Surge 侧 `v3` / `v3.1` 两版 ×2 + 懒人版 `lazy_v1.0` 快照。归档**不参与任何检查**，当前版永远只有顶层那四件；沿革见 `egern/docs/07` |
