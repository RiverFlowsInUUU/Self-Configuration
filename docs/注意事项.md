...
| 🤖 **agent 的入口在仓根** | [`AGENTS.md`](../AGENTS.md) 是这一节的浓缩版：任务分流、改完的固定动作（`bash skill/tests/all.sh` 判五项 · 落地后 `--landed` 判三数）、五条硬约束（不加 workflow / 不引用前身仓 / 别为行尾改本机 git 配置 / 判"不对"前先 grep `audit-waive` / **改那 10 个「闸门文件」前先请示**）。换设备时你只需要把仓库地址交给 agent，其余它自己读得到 |
...
| 🔁 **改完的固定动作** | `bash skill/tests/all.sh` 五项判据全过 → `git add -A && git commit && git push` → `bash skill/tests/all.sh --landed` 复核这批。`all.sh` 末尾报「未提交 N 个 · 落后 N · 未推送 N」，双端是否一致只看这三个数：`0 / 0 / 0` —— **默认档只报不判**（那一刻还没提交，红了也说明不了问题），判它的是 `--landed` |
