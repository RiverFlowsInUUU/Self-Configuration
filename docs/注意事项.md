...
| 🧪 **全部本地完成，刻意不挂 CI** | 改完在仓库根目录跑 **一条命令**：`bash skill/tests/all.sh`（内含两内核回归、全仓 markdown 链接与锚点、两版形态去注释对拍、换设备可移植性、文档读数与实测对拍、工具自检（自带回归与 `.py` 可编译））。要单跑某一侧时用 `bash skill/tests/surge/run.sh` / `bash skill/tests/egern/run.sh`。退出码 `0` 全绿 · `1` 有判负 · `2` 前置不达标（缺 Python / 缺 PyYAML / 顶层固定名四件有缺失）—— `2` 刻意与 `1` 分开：**没跑成不等于跑绿**。环境要 **Python 3**，Egern 侧脚本还需 **PyYAML**（Windows 的 Git Bash 不自带，需自行安装）。这六项检查**只看本仓库的文件**：仓库根由脚本自身位置反推，clone 到哪、叫什么名字都一样跑，不需要任何其他仓库在场。命令与环境要求见 [`../skill/reference/surge/checker.md`](../skill/reference/surge/checker.md) |
...
| 🤖 **agent 的入口在仓根** | [`AGENTS.md`](../AGENTS.md) 是这一节的浓缩版：任务分流、改完的固定动作（`bash skill/tests/all.sh` 判六项 · 落地后 `--landed` 判三数）、五条硬约束（不加 workflow / 不引用前身仓 / 别为行尾改本机 git 配置 / 判"不对"前先 grep `audit-waive` / **改那 10 个「闸门文件」前先请示**）。换设备时你只需要把仓库地址交给 agent，其余它自己读得到 |
...
| 🔁 **改完的固定动作** | `bash skill/tests/all.sh` 六项判据全过 → `git add -A && git commit && git push` → `bash skill/tests/all.sh --landed` 复核这批。`all.sh` 末尾报「未提交 N 个 · 落后 N · 未推送 N」，双端是否一致只看这三个数：`0 / 0 / 0` —— **默认档只报不判**（那一刻还没提交，红了也说明不了问题），判它的是 `--landed` |
