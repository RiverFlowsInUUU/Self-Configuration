...
| 🤖 **agent 的入口在仓根** | [`AGENTS.md`](../AGENTS.md) 是这一节的浓缩版：任务分流、改完的固定动作（`bash skill/tests/all.sh` 判七项 · 落地后 `--landed` 判三数）、五条硬约束（不加 workflow / 不引用前身仓 / 别为行尾改本机 git 配置 / 判"不对"前先 grep `audit-waive` / **改那 10 个「闸门文件」前先请示**）。换设备时你只需要把仓库地址交给 agent，其余它自己读得到 |
