...
| `direct.txt` | 国内域名（实测 111,171 条，零 IP）**主承重墙** | `DIRECT`（同上，不写开关） | `DIRECT` | [Loyalsoldier/surge-rules](https://github.com/Loyalsoldier/surge-rules) |
...
| ~~`domain_suffix: cn`~~ | 两侧均已删除 | 2026-09-24 懒人版也删了。理由与分流版 v3 同：`direct.txt` 本身含 `DOMAIN-SUFFIX,cn`（实测第 23,829 行），留着只会把「规则集没接住」掩盖成「cn 直连正常」 |
