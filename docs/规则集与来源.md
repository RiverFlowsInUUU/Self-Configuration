...
⚠️ **懒人版两侧并不同构**：Surge `lazy.conf` 含 `SYSTEM` / Apple 两条，Egern `lazy.yaml` 含
`domain_suffix: cn` 一条。默认出口两侧现在**都是 `Proxy`**（Egern 侧的 `Final` 中间组
2026-09-23 移除）。内网两侧都是两条
（Surge 内置 `LAN` + `private.txt` ／ Egern `Lan.list` + `Private`，后者 2026-09-23 补齐）。逐项差异见
...
**懒人版**：Surge 11 条 / Egern 10 条，顺序与成员均有差异，见两侧 profile 文件内注释。
