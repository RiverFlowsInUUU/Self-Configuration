# 规则集与来源

> 这一页是组件清单：用了哪些规则集、各自从哪来、按什么顺序生效。
> 两内核共用同一批远程规则集（**21 个 URL 逐字相同**），差异只有 1 个 Egern 独有项与 2 个内置项。

## 1 · 共用规则集

两侧都是 **Surge 原生 `.list` / `.txt` 格式**，由客户端自行下载与缓存。
Egern 的规则集**内联在 `rules` 段的 `rule_set` 条目里**，没有独立 `rule_sets` 段。
`update-interval=86400` / `update_interval: 86400` 表示 24 小时刷新，**两侧都必须显式写**——缺省时规则集可能停在首次下载的版本，上游新收录的域名会一直命不中。

| 规则集 | 作用 | Surge 去向 | Egern 去向 | 来源 |
|:-------|:-----|:-----------|:-----------|:-----|
| `surge-white-guard.list` | 白名单（精确域名） | `DIRECT` | `DIRECT` | [Jinx](https://github.com/RiverFlowsInUUU/Jinx) |
| `surge-ads.list` | 广告拦截主清单 | `REJECT`（`pre-matching`） | `AD` 组 | [Jinx](https://github.com/RiverFlowsInUUU/Jinx) |
| `AWAvenue-Ads-Rule-Surge-RULE-SET.list` | 广告拦截第 2 条 | `REJECT`（`pre-matching`） | `AD` 组 | [TG-Twilight/AWAvenue-Ads-Rule](https://github.com/TG-Twilight/AWAvenue-Ads-Rule) |
| `private.txt` | 特殊 TLD 与路由器域（`.lan` `.local` `miwifi.com`） | `DIRECT` `no-resolve` | `DIRECT`（分流版 v3 起补） | [Loyalsoldier/surge-rules](https://github.com/Loyalsoldier/surge-rules) |
| `direct.txt` | 国内域名（约 11 万条）**主承重墙** | `DIRECT` `no-resolve` | `DIRECT` | [Loyalsoldier/surge-rules](https://github.com/Loyalsoldier/surge-rules) |
| `OpenAI.list` | → `ChatGPT` | ✅ | ✅ | [blackmatrix7/ios_rule_script](https://github.com/blackmatrix7/ios_rule_script) |
| `Gemini.list` | → `Gemini` | ✅ | ✅ | 同上 |
| `Anthropic.list` · `Claude.list` | → `Claude` | ✅ | ✅ | 同上 |
| `AI.list` | → `AI`（通用 AI，必须排在上面三条**之后**） | ✅ | ✅ | [ACL4SSR/ACL4SSR](https://github.com/ACL4SSR/ACL4SSR)（固定 commit） |
| `Spotify.list` · `YouTubeMusic.list` · `YouTube.list` | 各自应用组 | ✅ | ✅ | [blackmatrix7/ios_rule_script](https://github.com/blackmatrix7/ios_rule_script) |
| `GitHub.list` · `Google.list` · `Microsoft.list` | 各自应用组 | ✅ | ✅ | 同上 |
| `Telegram.list` · `Twitter.list` · `WeChat.list` | 各自应用组 | ✅ | ✅ | 同上 |
| `Apple_All_No_Resolve.list` | Apple 服务 → `DIRECT` | ✅ | ✅ | 同上（**必须用 `No_Resolve` 变体**，见第 4 节） |
| `Proxy.list` | 常用代理名单 | 注释态，不参与匹配 | `disabled: true`，不生效 | 同上 |
| `GEOIP,CN` / `geoip: CN` | 国内 IP 段 | `DIRECT` `no-resolve` | `DIRECT` `no_resolve` | Surge：`GeoLite2-Country.mmdb` · [adysec/IP_database](https://github.com/adysec/IP_database)；Egern：`Country.mmdb` + `GeoLite2-ASN.mmdb` · [Loyalsoldier/geoip](https://github.com/Loyalsoldier/geoip) |

## 2 · 只有一侧有的

| 规则集 | 只在哪侧 | 原因 |
|:-------|:---------|:-----|
| `LAN`（内置） | 仅 Surge | Surge 有内置局域网规则集 |
| `Lan.list` | 仅 Egern | Egern 无内置 LAN，引用 [blackmatrix7](https://github.com/blackmatrix7/ios_rule_script) 的清单补位。与 `private.txt` **互补**：`Lan` 管 IP 段与路由管理域，`private` 管特殊 TLD 与 `miwifi.com` |
| `SYSTEM`（内置） | 仅 Surge | 系统进程 / 系统服务 → `DIRECT`。**Egern 没有对应内置集**，该位置放着注释态的 `Proxy.list` |
| `domain_suffix: cn`（手写） | 仅 Egern 懒人版 | 不依赖规则集是否下载成功的 `.cn` 兜底。Egern 分流版 v3 已删（`direct.txt` 本身含 `DOMAIN-SUFFIX,cn`，属重复），**懒人版仍保留** |

⚠️ **懒人版两侧并不同构**：Surge `lazy.conf` 含 `SYSTEM` / Apple / `private.txt` 三条，Egern `lazy.yaml` 含
`domain_suffix: cn` 一条；默认出口一个是 `Proxy` 一个是 `Final`。逐项差异见
[`跨内核差异对照.md`](跨内核差异对照.md) 第 4 节。

## 3 · 匹配顺序

自上而下，第一条命中即决定去向。分流版两侧 24 条**逐位对齐**（仅第 3 节的三处引擎差异例外）。

**分流版（两侧共用基准序，位 ①–㉔）**

| 位 | 内容 | 位 | 内容 |
|:-:|:-----|:-:|:-----|
| ① | 白名单 `surge-white-guard.list` → `DIRECT` | ⑬–⑱ | `YouTube` · `GitHub` · `Google` · `Microsoft` · `Telegram` · `Twitter` 各应用组 |
| ②③ | 两条广告清单 → `REJECT`(S) / `AD`(E) | ⑲ | Surge `SYSTEM` → `DIRECT` ／ Egern `Proxy.list`（disabled） |
| ④⑤ | 内网：Surge `LAN` · `private` ／ Egern `Lan.list` · `Private` | ⑳ | `Apple_All_No_Resolve.list` → `DIRECT` |
| ⑥–⑩ | 厂商专属在前：`OpenAI` · `Gemini` · `Anthropic` · `Claude`，再 `AI.list` | ㉑ | `WeChat.list` → `WeChat` |
| ⑪⑫ | `Spotify` · `YouTubeMusic` | ㉒–㉔ | `direct.txt` → 国内 IP 判定 → **默认出口** |

**懒人版**：Surge 11 条 / Egern 9 条，顺序与成员均有差异，见两侧 profile 文件内注释。

## 4 · 排序与选材约束（改动前逐条确认）

1. **白名单必须排在最前**，且在两条广告清单之前 —— Jinx 与 AWAvenue 存在重叠域名，白名单排到后面会被误杀。
2. **厂商专属规则（`OpenAI` / `Gemini` / `Anthropic` / `Claude`）排在 `AI.list` 之前** —— 否则 AI 域名先被 `AI.list` 接走，专属组形同虚设。
3. **`GitHub.list` 排在 `direct.txt` 之前** —— `github.com` 同时被国内直连清单收录，排到后面就接不到它。
4. **IP 类规则排最后，且必须带 `no-resolve` / `no_resolve`** —— 否则每个走到它的域名都会被强制本地解析一次，那正是泄露来源。详见 `surge/docs/05` · `egern/docs/05`。
5. ⭐ **用 `*_No_Resolve` 变体，并下载下来数条目**。`Apple_All.list` 实测含 13 条不带 `no-resolve` 的裸 IP（Apple CDN 网段），
   一条这样的条目就让**每个走到该规则的域名**都被强制解析一次。判据是"下载 + 数条目"，**不是看规则集名字**
   （`ChinaMax.list` 名字像域名集，实测 12472 条里只有 64 条域名）。
6. **两条广告清单的策略与参数必须逐字相同**，否则同一广告域名因命中不同清单而进出不一致。

> 🛡️ Egern 侧保留 `domain_suffix: cn` 的理由值得记：**不依赖规则集加载成功**——规则集下载失败时 `.cn` 仍能直连。

## 5 · 素材与许可

| 素材 | 用途 | 来源 |
|:-----|:-----|:-----|
| 策略组图标 26 个 | 面板图标（两内核共用同一份） | [RiverFlowsInUUU/Rule](https://github.com/RiverFlowsInUUU/Rule) · [jnlaoshu/MySelf](https://github.com/jnlaoshu/MySelf) · [Koolson/Qure](https://github.com/Koolson/Qure) |
| `GeoLite2-Country.mmdb` | Surge 侧 `GEOIP` 判定 | [adysec/IP_database](https://github.com/adysec/IP_database) |
| `Country.mmdb` · `GeoLite2-ASN.mmdb` | Egern 侧 `geoip` / `asn` 判定 | [Loyalsoldier/geoip](https://github.com/Loyalsoldier/geoip) |

许可见 [`图标与许可.md`](图标与许可.md)。第三方规则集版权归其原作者。

---

相关：[`跨内核差异对照.md`](跨内核差异对照.md) · [`注意事项.md`](注意事项.md) · [`../README.md`](../README.md)
