...
- 🛡️ **闸门那句 ✅ 话改准 + 放行例外写进规则 + 最后一处循环盘引用去指（q13 · 用户明文授权三项）**：
  `all.sh` 原来那句「✅ 闸门未被动过」**说得过头**——它查的只是未提交与未推送的提交两个面（该仓无上游时退到最近一次提交），
  不查已落地的历史；现改成「✅  本批待落地改动未触碰其他闸门文件（查：未提交 + …）」，无上游那面的点名也跟着写成
  「最近一次提交（该仓无上游）」。
  `AGENTS.md` §2 第 5 条补上那句到底查哪几个面、以及**唯一放行例外 `once_released()` 的现状**（只认该件正文里的授权标记字面、
  标记被删之前对这一件持续放行、放行时另出一行 ℹ️ 点名该件），末句写死后来者不许照学这个模式开后门。
  `skill/tests/surge/check_links.py` 头注最后一处指向循环协作盘取数件的散文引用去指，改成不带盘路径的同源表述。
  **没动**：`GATE` 那 12 件名单本身 · 三处写死个数（AGENTS.md §2 名单块与标题 · `docs/注意事项.md` 浓缩句 ·
  `apply_edits.py` 头注）· 判据 ①② 本体与 `--selftest` 两 fixture · 放行例外的**行为**（"给豁免加 HEAD 失效维"经 B 判
  超出本轮三项授权，用户已令另立 q14 单做，本批不含）· 授权标记 `q11-user-approved` 与 132 数值逐字保留 ·
  旧措辞在本日志与 `docs/日志旧版原文.md` 里的沿革引用不改写 · 根 README · manual · profile · 其他一切跟踪树文件。
  **验收**（写死本条落地 commit 的父件 `d10d005`，现势可跑）：把 `skill/tests/all.sh` 里 `<<PYEOF` 与 `PYEOF` 之间那段
  Python 原样抽出，在四种临时仓形上各跑一遍 ⇒ ①有上游 · 已推送 · 干净 =「✅ 本批待落地改动未触碰其他闸门文件
  （查：未提交 + 未推送的提交 · 冻结 12 个文件…）」；②无上游 · 最近一次提交动了 `all.sh` =「⚠️ 闸门被改动 1 处」
  + 点名行「最近一次提交（该仓无上游）」；③有上游 · 未推送提交动了 `all.sh` = 同上，点名为「未推送的提交」；
  ④未推送提交动了 `check_links.py`（豁免生效中）= ℹ️ 放行一行与 ✅ 行**同屏不再互斥**（✅ 说的是"其他"）。
  全仓搜"循环协作盘那个点开头目录名"的字面引用：改前 1 行 ⇒ 改后 **0** 行；
...
- 📚 **两份版本沿革件与手册的六处读数修正（q21）**：上一批只换了版本标签、沿革正文停在旧读数，本轮逐处改到现状 ——
  `egern/docs/07:82` 原写「本批新增 `[SSID Setting]` 段（Surge 侧同步）」，与事实相反：该段只在 Surge 侧，
  Egern 侧本批内容零改动（`grep -rn "SSID" egern/profiles/` 实测 0 命中），现改为「Egern 无对等件
  （只有 `rules` 的 `ssid` 条件项，「暂停」≠「该网络下走指定策略」）· 差异登记见 `docs/跨内核差异对照.md` §1
  网络级暂停」，并在 `:83` 补一句 Egern 为何同批升版（`bump_version.py:238–240` 判「两内核头注不一致，
  升版会把分叉固化」⇒ 同族跨内核四件必须同号）；一览表 `:64` 新增 `routing_v3.3` 行、承接原先挂在当前版行上的
  v3.3 delta（`proxies` 由空列表 → 2 条占位节点等四条），`:65` 当前版 `routing_v3.4` 行改写为本批实际变化
  （内容零改动 · `[SSID Setting]` 属 Surge 侧 · `routing_v3.3` 两份进 `config_old/`）；`:190` 静态读数的跨度
  由「四版」改「五版」（`routing_v3` ~ `routing_v3.4`）；`:171` 懒人版谱系补 `lazy_v1.3` 一行（同为内容零改动、
  只升头注）；`surge/docs/07` 新增 §3.8「第八版（`v3.4`）与懒人版 `v1.3`：模板自带 `[SSID Setting]` 网络级暂停」
  （`:201–215`，§3.7 与其后各节逐字节未动），存档表 `:119` 补 `routing_v3.3.conf` / `.min.conf` 一行；
  `manual/03` §3.6 必须替换清单由 2 项增至 3 项（`:129` 补两份 `[SSID Setting]` 里的 `SSID:MyHome` ——
  不替换就匹配不到任何 Wi-Fi，「回家自动暂停」静默不生效）。
  **没动**：四份 profile 本体与 `config_old/` 归档零字节；`surge/docs/04`（模板逐段讲解 · 段序已冻结）不插 SSID 小节，
  该段的逐段讲解由手册层 `manual/03` §3.6 承担；带日期的旧日志条目、以及首行标注「不随现状更新」的存档里的旧读数
  一律不回改。
  **验收**：`bash skill/tests/all.sh` ⇒ 「✅ 判据全过 · 7 项」（19 / 24 / 18 / 18 / 18 / 8 / 6 · `failed` 全 0）·
  闸门行「未触碰闸门文件」；`check_links.py` rc 0 ⇒ 扫描 59 件 / 现役层命令行完整路径 132 处逐字不变，
  相对链接 462 → 466（本批新增 4 条交叉链接：`egern/docs/07:82`、`surge/docs/07:208`、`surge/docs/07:214`、
  `manual/03:129`，全部可解析）；改动的三份文面 CR = 0；改前改后对拍全走非破坏式 `git show 14a7e6b:<path>`。
