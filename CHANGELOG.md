...
- 📚 **新增 [`docs/技能包合并与自包含.md`](docs/技能包合并与自包含.md)**：装配脚本退役前，把它背后的规则
...
- 🩺 **全仓体检报告进仓**（[`docs/体检报告.md`](docs/体检报告.md)）：P1–P11 的完整记录，含三处
  **被实测推翻的原结论**。去掉了本机绝对路径与仓外目录名，对前身仓的评价改为本仓视角，
  当时的错误判断保留并标注 —— 这类记录的价值恰恰在于"错在哪、怎么被纠正"。
- 🧮 **新增仓根 [`.gitattributes`](.gitattributes)：`* text=auto eol=lf`（新发现 P11 的修法）**。
...
- 🐛 **修掉 `E2` 的一个真盲区 + 补一条老克隆的迁移动作**：`is_text()` 只认扩展名白名单 ⇒
  **`LICENSE` 这类无扩展名文本文件从没进过字节判据**（同一个老克隆里独立测得 94 个 CRLF，检查只报 93）。
  ...
  这条已写进 [`docs/注意事项.md`](docs/注意事项.md) 的「换设备 / 双端一致」一节。
- 🤖 **新增仓根 [`AGENTS.md`](AGENTS.md)：给 AI agent 的开工说明**。此前"换设备可无缝"的线索只散在
