...
- 🛠 新增批量编辑执行器 [`skill/tests/apply_edits.py`](skill/tests/apply_edits.py)：一份 JSON 说清「哪些文件、把哪句改成哪句」，
  ...
  含 CR 的文件拒改、**冻结闸门 9 个文件默认拒绝**（要 `--allow-gate`，拒绝消息里写着请示要附哪两条）。
...
- 🚪 **本轮触碰闸门 2 个文件**：`all.sh`（加第 5 项命令与抬头注释「五项」）· `bump_version.py`（上一条的排除名单与文案）。
  ...
  那两份是证据、不是活指向。`check_doc_readings.py` 与 `apply_edits.py` 两份新脚本**没有**加进冻结名单。
